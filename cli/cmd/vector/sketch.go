...
// runSpecAttachSketch implements `vector spec attach-sketch <id> --file <path>`:
...
func runSpecAttachSketch(args []string) error {
	...
	fs := flag.NewFlagSet("spec attach-sketch", flag.ContinueOnError)
	idFlag := fs.String("id", "", "spec id to attach the sketch to (optional; or pass it as the first argument)")
	file := fs.String("file", "", "path to the .excalidraw JSON to attach (required)")
	name := fs.String("name", "", "stored file name (defaults to the base name of --file)")
