...
func runStandup(args []string) error {
	if len(args) > 0 && args[0] == "commit" {
		return runStandupCommit(args[1:])
	}
	fs := flag.NewFlagSet("standup", flag.ContinueOnError)
	since := fs.String("since", "", "window: 24h|today|7d (default: since the last standup marker)")
	repoRoot := fs.String("repo-root", "", "repo root (defaults to git toplevel or cwd)")
	jsonOut := fs.Bool("json", false, "emit the projection as JSON for the digest agent")
...
func runStandupCommit(args []string) error {
	fs := flag.NewFlagSet("standup commit", flag.ContinueOnError)
	digestFile := fs.String("digest-file", "", "path to the digest JSON, or - for stdin (required)")
	since := fs.String("since", "", "window the digest covers: 24h|today|7d (default: since the marker)")
	repoRoot := fs.String("repo-root", "", "repo root (defaults to git toplevel or cwd)")
...
func runSpecWorklog(args []string) error {
	id, rest := leadingID(args)
	fs := flag.NewFlagSet("spec worklog", flag.ContinueOnError)
	idFlag := fs.String("id", "", "spec id (or pass it as the first argument)")
	files := fs.String("files", "", "comma-separated files touched")
	tasks := fs.String("tasks", "", "comma-separated tasks completed")
	note := fs.String("note", "", "short note (truncated to 280 chars)")
